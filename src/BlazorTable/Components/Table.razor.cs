...
namespace BlazorTable {
	...
	public partial class Table<TableItem> : ComponentBase, ITable<TableItem> {
		...
		/// </summary>
		[Parameter]
		public string TableContainerClass { get; set; } = "card mb-3 border-0";
		...
		/// </summary>
		public int PageFirstItemNumber { get; private set; }

		/// <summary>
		/// The last record index for the current page.
		/// </summary>
		public int PageLastItemNumber { get; private set; }
		...
		private IEnumerable<TableItem> GetData() {
			...
			if (Items != null || ItemsQueryable != null) {
				...
				PageFirstItemNumber = (PageNumber * PageSize) + 1;
				var pageTo = (PageNumber * PageSize) + PageSize;
				PageLastItemNumber = Math.Min(TotalCount, pageTo);
		...
		private readonly Dictionary<int, bool> detailsViewOpen = new Dictionary<int, bool>();
		...
		/// </summary>
		/// <param name="optionsOrColumnComponent"></param>
		public void AddColumn(IColumnOptions<TableItem> optionsOrColumnComponent) {

			if (optionsOrColumnComponent is Column<TableItem>) {
				throw new Exception("This method does not accept a Column Component. Please use ColumnOptions<T>.");
			}

			var columnComponent = new Column<TableItem>();
			columnComponent.Init(optionsOrColumnComponent);

			AssociateColumn(columnComponent);

		}

		internal void AssociateColumn(IColumn<TableItem> column) {
			...
			switch (SelectionType) {
				...
				case SelectionType.Multiple:
					if (SelectedItems.Contains(tableItem)) {
						_ = SelectedItems.Remove(tableItem);
					} else {
						SelectedItems.Add(tableItem);
					}
					break;
		...
		private Expression<Func<TableItem, bool>> GlobalSearchQuery(string value) {
			...
			foreach (string keyword in value.Trim().Split(" ")) {
				...
				expression = expression == null ? tmp : expression.And(tmp);
			}
